...
#           V.12 - Added Error messages in boxes
...
#    log = LogWindow(log_window)
#    log.show_log()
#
#class LogWindow:
#    def __init__(self, master):
#        self.master = master
#        self.log_text = scrolledtext.ScrolledText(self.master, width=80, height=30)
#        self.log_text.pack()
#
#    def show_log(self):
#        cmd = 'python script_pioth_v12.py'
...
#            if error:
#                self.log_text.insert(tk.END, error)
#            self.log_text.see(tk.END)

#class PrintWindow:
#     def __init__(self, master):
#         self.master = master
#         self.print_text = tk.Text(self.master, width=150, height=30)
#         self.print_text.pack()
#
#     def write(self, text):
#        self.print_text.insert(tk.END, text)
#
#     def flush(self):
#        pass
#
#def update_print_window(print_window, text_widget):
#    # Get the new output from stdout
#    new_output = sys.stdout.printer.print_text.get(print_window.last_pos, tk.END)
#
#    # Update the last_pos marker
#    print_window.last_pos = tk.END
#
#    # Update the text widget with the new output
#    text_widget.insert(tk.END, new_output)
#
#    # Reschedule this method to be called again after a delay
#    text_widget.after(100, update_print_window, print_window, text_widget)
#
#def display_print_window():
#    print_window = tk.Toplevel()
#    print_window.title('Print Output')
#    print_window.geometry('1000x500')
#    print_window.resizable(False, False)
#    print_window.lift()
#    #print_window.attributes('-topmost', False)
#    print_window.focus_force()
#
#    print_window.printer = PrintWindow(print_window)
#    sys.stdout = print_window.printer
#     # Set the initial value of last_pos
#    print_window.last_pos = tk.END
#    # Start the update loop
#    update_print_window(print_window, print_window.printer.print_text)
...
#
        ...
        #console_text = tk.Text(frame, bg='black', fg='white', state='disabled')
        ...
        ## create log button
        #log_button  = tk.Button(frame, text = 'View Log', command=display_log_window, font = label_font)
        #log_button.configure(bg="#003568", fg="#c7c8ca", activebackground="#c7c8ca", activeforeground="#003568")
        #log_button.pack(pady=(10,0))

        ## create print button
        #print_button = tk.Button(frame, text='View Print Output', command=display_print_window, font=label_font)
        #print_button.configure(bg="#003568", fg="#c7c8ca", activebackground="#c7c8ca", activeforeground="#003568")
        ...
        output = scrolledtext.ScrolledText(frame, wrap=tk.WORD, height=10)
        output.pack(pady=(10, 0))

        ### redirect console output to a pipe
        ##pipe = sys.stdout = sys.stderr = sys.stdin = os.popen('tee', 'w')
    ##
        ### start a new thread to continuously update the output text widget
        ##Thread(target=update_output).start()
